(function() {
  ...
  parse = function(source, code) {
    ...
    // a real world closure: build an array of 
    // {docs_text:, code_text:} objects with doc and code strings
    ...
    try {
    // iterate over lines and matches just inline comments
      for (_i = 0, _len = lines.length; _i < _len; _i++) {
        line = lines[_i];
        if (line.match(language.comment_matcher) && !line.match(language.comment_filter)) {
          if (has_code) {
        	// save a single line of comment 
        	// and source seen till now
            save(docs_text, code_text);
            has_code = docs_text = code_text = '';
          }
          docs_text += line.replace(language.comment_matcher, '') + '\n';
        } else {
          // if it is not a line of comment it is a line of code. Put it apart
          // (it will be saved at next comment found)
          has_code = true;
          code_text += line + '\n';
        }
      }
      // last line is probably not a comment, so we save the remainder
      // out of the for
      save(docs_text, code_text);
    } catch (e) {
  ...
  highlight = function(source, sections, callback) {
    ...
    language = get_language(source);
    // spawn a process and run pygmentize
    pygments = spawn('pygmentize', ['-l', language.name, '-f', 'html', '-O', 'encoding=utf-8']);
    output = '';
    // use listener on stderr (data event) and stdin (error event) to make error reporting
    ...
    pygments.addListener('exit', function() {
      // HTML conversion of both code and comments section by section
      // opening and closing HTML is stripped and applied for each 
      // section
    ...
    // trigger all previously defined listener by
    // pushing the source into stdin of previously
    // spawned process
  ...
  generate_html = function(source, context, sections) {
    ...
    // HTML title is filename
    title = path.basename(source); 
    // TODO return output filename
    ...
    // function that actually writes the data
    write_func = function() { 
    ...
    // create it if necessary and then write the HTML on file
  ...
  // import just some functions in local namespace
  _ref = require('child_process'), spawn = _ref.spawn, exec = _ref.exec;
  // language definitions, mapped to filename extensions
  languages = {
    '.coffee': {
      name: 'coffee-script',
      symbol: '#'
  ...
  // get language definition from file extension
  ...
  template = function(str) {
    return new Function('obj', 'var p=[],print=function(){p.push.apply(p,arguments);};' + 'with(obj){p.push(\'' + 
    		str.replace(/[\r\t\n]/g, " ").replace(/'(?=[^<]*%>)/g, "\t").split("'").join("\\'").split("\t").join("'").
    		replace(/<%=(.+?)%>/g, "',$1,'").split('<%').join("');").split('%>').join("p.push('") + "');}return p.join('');");
  };
